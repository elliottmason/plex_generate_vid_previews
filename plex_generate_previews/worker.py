...
class WorkerPool:
    ...
    def process_items(self, media_items: List[tuple], config: Config, plex, worker_progress, main_progress, main_task_id=None, title_max_width: int = 20, library_name: str = "") -> None:
        ...
        # Track starting stats so cross-library counts don't prematurely satisfy exit conditions
        ...
        while True:
            ...
            if not media_queue:
                ...
                actual_completed = sum(worker.completed for worker in self.workers) - start_completed
                actual_failed = sum(worker.failed for worker in self.workers) - start_failed
                # Guard against negative values if counters reset unexpectedly
                actual_processed = max(actual_completed, 0) + max(actual_failed, 0)
